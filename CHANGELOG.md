...
- 锁定四个 KWeaver 上游仓库及所选组件的不可变提交和处置结论；
- 在 Go 1.25.12 隔离环境通过 BKN backend、ontology-query 和 Vega backend 全模块测试；
- 通过官方 KWeaver Python SDK 0.8.4 增加知识网络列表、详情、创建和全量构建防腐层与 API；
- 增加知识网络工作台，查询、刷新、创建、查看详情和触发构建均连接真实后端用例。
- 增加经营对象类型列表与创建，支持字段、类型、主键和显示字段的真实 KWeaver 持久化；
- 增加独立 Vega 经营对象目录、托管数据集和创建失败补偿清理；
- 增加对象类型真实 API、BKN 重启持久化和浏览器布局验收记录。

### Changed

- 产品入口由 0.4.0 决策矩阵探索页改为 KWeaver 业务知识网络工作台；
- 生产镜像从固定 Git 提交构建 SDK，并在安装后移除 Git 工具和临时依赖。
- 从经营用户界面移除 Core 不支持的资源直连对象全量构建按钮，并清理相关概念文案。

### Deprecated

- 0.4.0 经营方案加权比较保留为历史需求探索，不再作为目标产品领域模型或首页。
